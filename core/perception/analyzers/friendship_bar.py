...
class FBAConfig:
    ...
    cap_ignore_frac: float     = 0.12 # ignore rightmost gray “cap” when measuring color/fill
    ...
    col_majority_threshold: float = 0.40  # a column counts as filled if >=40% colored pixels
    ...
    min_color_cover: float = 0.05     # require at least 5% area for a color to be considered
...
class FriendshipBarAnalyzer:
    ...
    def _bar_strip_xyxy(self, roi_xyxy: Tuple[int, int, int, int]) -> Tuple[int, int, int, int]:
    ...
    def _vote_dominant_color(self, hsv: np.ndarray, drop_right_cols: int) -> Dict[str, float]:
        ...
        blue_mask   = colored & (self._circular_hue_distance(H, self.cfg.blue_h_center)   <= tol)
        green_mask  = colored & (self._circular_hue_distance(H, self.cfg.green_h_center)  <= tol)
        orange_mask = colored & (self._circular_hue_distance(H, self.cfg.orange_h_center) <= tol)
        ...
        covers = {
            ...
            "yellow": yellow_cover,
            "colored_cover": float(total) / max(1, H.size) # overall fraction of colored pixels
        }
        return covers
    ...
    def analyze(self,
        ...
        if is_max:
            ...
            decisive = (v1 >= self.cfg.min_color_cover) and ((v1 - v2) >= self.cfg.vote_margin)
