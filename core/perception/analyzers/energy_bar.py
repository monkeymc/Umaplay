...
def energy_from_bar_crop(pil_img: Image.Image) -> Dict[str, object]:
    ...
    H, W = bgr.shape[:2]
    if H < 10 or W < 20:
        return {"energy_ratio": 0.0, "energy_pct": 0, "gray_ratio": 1.0,
                "valid": False, "reason": "too-small", "inner_shape": (W, H)}
    ...
    if inner.size == 0:
        return {"energy_ratio": 0.0, "energy_pct": 0, "gray_ratio": 1.0,
                "valid": False, "reason": "inner-crop-empty", "inner_shape": (0, 0)}
    ...
    if x2c - x1c < max(10, int(0.3 * W)):
        return {"energy_ratio": 0.0, "energy_pct": 0, "gray_ratio": 1.0,
                "valid": False, "reason": "content-not-found",
                "x1_content": x1c, "x2_content": x2c, "inner_shape": (inner.shape[1], inner.shape[0])}
    ...
    # cut position
    if filled_cols > 0:
        # last colored-majority column index in original (inner) coordinates
        if not (colored_major == False).any():
            cut_x = x1c + total_cols - 1
        else:
            cut_local = int(np.argmax(colored_major[::-1] == False))
            cut_x = (x2c - 1) - cut_local
    else:
        cut_x = x1c
